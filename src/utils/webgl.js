 ...
 * @returns {WebGLShader|null} 编译后的着色器，失败返回null
 */
 ...
 * @returns {WebGLProgram|null} 链接后的程序，失败返回null
 */
 ...
 * @param {string} fragmentSource - 片段着色器代码
 * @param {string[]} uniformNames - uniform变量名列表
 * @returns {Object} 包含gl、program和uniform位置的对象
 */
export function initShaderProgram(canvas, vertexSource, fragmentSource, uniformNames = []) {
 ...
 */
export function initPreviewShader(canvas, shaderId) {
  if (!canvas) return
 ...
 */
export function formatShaderCode(code) {
  // 移除首尾空行，保持缩进
  return code.trim()
}
